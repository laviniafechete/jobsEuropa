...
export default function JobList() {
  ...
  const [applying, setApplying] = useState<{ [jobId: string]: boolean }>({});
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('.contact-options-container')) {
        setShowContactOptions({});
        setShowModalContactOptions(false);
      }
    };
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2 mt-4">
                ...
                {job.hasApplied ? (
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); setSelectedJob(job); setShowModal(true); }}
                    disabled={applying[job._id] || (user?.hasCompletedCv === false)}
                    className={`flex items-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white px-4 py-2 rounded-lg transition-colors${user?.hasCompletedCv === false ? ' opacity-60 cursor-not-allowed' : ''}`}
                    style={{ minWidth: 180 }}
                    title={user?.hasCompletedCv === false ? 'Trebuie să ai CV-ul completat pentru a aplica la joburi' : ''}
                  >
                    {applying[job._id] ? (
                      <>
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); setSelectedJob(job); setShowModal(true); }}
                  className="ml-2 flex items-center gap-1 px-3 py-2 border border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 text-sm"
                >
                  Detalii
                </button>
              </div>
          ...
          >
            ...
            >
              ...
              <div className="flex justify-center mt-6">
                <div className="relative contact-options-container">
                  <button
                    onClick={(e) => { e.stopPropagation(); setShowModalContactOptions(!showModalContactOptions); }}
                    className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg transition-colors"
                  >
                    <MessageCircle size={16} />
                    Contactează-ne
                    <ArrowRight size={16} className={`transform transition-transform ${showModalContactOptions ? 'rotate-90' : ''}`} />
                  </button>
                  
                  {showModalContactOptions && (
                    <div className="absolute top-full left-0 mt-2 w-full bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                      <div className="p-2 space-y-1">
                        <button
                          onClick={(e) => { e.stopPropagation(); handlePhoneContact(selectedJob); }}
                          className="w-full flex items-center gap-3 px-3 py-2 text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"
                        >
                          <Phone size={16} className="text-green-600" />
                          <span className="text-sm">Sună acum</span>
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); handleWhatsAppContact(selectedJob); }}
                          className="w-full flex items-center gap-3 px-3 py-2 text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"
                        >
                          <MessageCircle size={16} className="text-green-500" />
                          <span className="text-sm">WhatsApp</span>
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); handleEmailContact(selectedJob); }}
                          className="w-full flex items-center gap-3 px-3 py-2 text-gray-700 hover:bg-gray-50 rounded-md transition-colors text-left"
                        >
                          <Mail size={16} className="text-blue-600" />
                          <span className="text-sm">Trimite email</span>
