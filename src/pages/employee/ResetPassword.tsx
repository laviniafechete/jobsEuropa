...
const EmployeeResetPassword: React.FC = () => {
  ...
  const verifyToken = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5001/api'}/auth/verify-reset-token/user/${token}`);
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5001/api'}/auth/change-password-with-token/user/${token}`, {
        method: 'POST',
        headers: {
